...
class Item():
    def __init__(self, width, height, rate = 1, pos_x = None, pos_y = None):
        self.width = width
        self.height = height
        ...
        self.speed_x = self.width/10000 * random.uniform(4, 10) * rate * random.choice([-1, 1])
        self.speed_y = (-1) * self.height/2000 * random.uniform(4, 6) * rate
        self.g = height/30000 *rate
    ...
    def check(self):
        ...
        """
        if self.pos_x - self.width >= 0 and self.speed_x >= 0:
            return 1
        if self.pos_x <= 0 and self.speed_x <= 0:
            return 1
        if self.pos_y <= 0 and self.speed_y <= 0:
            return 1
        return 0
...
def main():
    ...
    time = 10
    items = [Item(1000, 800, rate = 3) for i in range(10)]
    while True:
        screen.fill(0)
