...
def percess_by_hsv(img,lower=np.array([0,131,180]),uper=np.array([6,255,255])):
    ...
    z = np.polyval(poly, dot_pos_x)
    #
    # plt.plot(dot_pos_y,dot_pos_x)
    # plt.plot(z,dot_pos_x)
    # plt.show()
    l = np.where(z == z.min())[0][0]
    b = np.where(z == z.max())[0][0]
    cv2.line(img, (int(z[l]), int(dot_pos_x[l])),(int(z[b]), int(dot_pos_x[b])),(0,255,0),20)
